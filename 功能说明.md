...
### v0.01
- 增加在五笔的基础上对拼音输出，实现形、音码混打
- 增加万象 super_sequence.lua 手工调频
- 优化 super_sequence.lua 对 wanxiang.lua 完全脱离引用、依赖形成独立模块
- 修复手工调频存放路径，目前存放在LUA目录中sequence.userdb

### v0.02
- 修复配置文件所有开关部份失效问题及优化配置代码
- 修复[显形,隐形]+[隐根,显根]+[显音,哑音]+[无声,有声](新增) new_spelling.lua 显示逻辑问题

### v0.03
- 增加 calculator.lua 计算器功能模块，完全脱离引用、依赖形成独立模块
- 增加五笔与拼音词库扩展(来自知频词库筛选出来的2W多词)

### v0.04
- 修复 calculator.lua 计算器功能模块无法使用小键盘数字键计算

### v0.05
- 修复方案无法使用 symbol.yaml 符号标识
- 删除五笔与拼音的词典中带有A-Z字母开头的词汇，避免输出时词库浑浊

### v0.06
- 增加 pair_punct.lua 智能标点功能，自动补齐成对符号
- 修复符号可按回车上屏以及修复单、双引号无法上屏等问题

### v0.07
- 增加 schedule.lua 日程模块，支持 /jc、/jq、/gz、/date 等命令
- 删除 wubi_pinyin.schema.yaml 自造词配置相关

### v0.08
- 增加 super_tips.lua 超级注释tips功能

### v0.09
- 增加来自知频2-4字词条，分别整合到五笔与拼音的词典
- 修复纯五笔无法自造词问题
- 修复五笔拼音无法读取自造词 user_coined_ext.txt 自定义编码输出文字
- 修复五笔、拼音方案在任务栏无法加载图标问题

### v0.10
- 增加 ce_trans 翻译功能以及 translate_merge.lua 过滤器 / english_translator.lua 翻译器
...
- 精简代码逻辑，移除未使用的代码
- 优化译关状态下自动清空翻译字典缓存
- 引入全局共享开关状态标记，译关状态下实现零查询开销

### v0.13
- 新增 smart_commit.lua 智能上屏处理器
- 新增自造词功能，使用 ` 键引导组合输入
- 优化 Submit_text.lua，同时提供处理器和翻译器功能
- 修复自造词翻译器全局注册问题
- 提高自造词权重至最高优先级
